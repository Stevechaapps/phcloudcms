...
type AiCtx = Context<{ Bindings: import("../cms/env.js").Env; Variables: import("../cms/env.js").Variables }>;
...
export function registerAiRoutes(app: App): void {
  const okOr = async (c: AiCtx, fn: () => Promise<Record<string, unknown>>) => {
    ...
    try {
      ...
      return e instanceof AiLimitError
        ? c.json({ error: "Daily free AI budget used up — try again tomorrow." }, 429)
        : c.json({ error: e instanceof AiUnavailableError ? e.message : "AI request failed" }, 503);
    }
  };
  ...
  app.post("/api/admin/ai/continue", async (c) => {
    ...
    return okOr(c, () => aiContinue(c.env, String(body.text ?? ""), guidelines).then((text) => ({ text })));
  });
  ...
  app.post("/api/admin/ai/summarize", async (c) => {
    ...
    if (!body) return c.json({ error: "Invalid JSON" }, 400);
    const guidelines = (await getSetting(c.env.DB, "ai_guidelines")) ?? "";
    return okOr(c, () => aiSummarize(c.env, String(body.text ?? ""), guidelines).then((text) => ({ text })));
  });
  ...
  app.post("/api/admin/ai/rewrite", async (c) => {
    ...
    const guidelines = (await getSetting(c.env.DB, "ai_guidelines")) ?? "";
    return okOr(c, () =>
      aiRewrite(c.env, String(body.text ?? ""), String(body.tone ?? "clear"), guidelines).then((text) => ({ text })),
    );
  });
  ...
  app.post("/api/admin/ai/titles", async (c) => {
    ...
    if (!body) return c.json({ error: "Invalid JSON" }, 400);
    const guidelines = (await getSetting(c.env.DB, "ai_guidelines")) ?? "";
    return okOr(c, () => aiTitles(c.env, String(body.content ?? ""), guidelines).then((titles) => ({ titles })));
  });
  ...
  app.post("/api/admin/ai/meta", async (c) => {
    ...
    const guidelines = (await getSetting(c.env.DB, "ai_guidelines")) ?? "";
    return okOr(c, () =>
      aiMeta(c.env, String(body.title ?? ""), String(body.content ?? ""), guidelines).then((m) => ({
  ...
  app.get("/api/admin/ai/usage", async (c) => {
    ...
    return c.json({ limit, remaining, used: limit - remaining, date: new Date().toISOString().slice(0, 10) });
  });
}